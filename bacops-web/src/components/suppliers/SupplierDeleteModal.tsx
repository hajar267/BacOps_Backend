...
export function SupplierDeleteModal({ item, onClose, onDeleted }: SupplierDeleteModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-text-primary/45 p-4">
      <div className="relative w-full max-w-sm rounded-xl bg-white p-6">
        ...
        <div className="mb-2 flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-text-primary">Supprimer le fournisseur</h2>
        </div>

        <p className="mb-6 text-sm text-text-secondary">
          Voulez-vous vraiment supprimer <span className="font-semibold text-text-primary">{item.nom}</span> ? Cette action est irréversible.
        </p>

        {error && <p className="mb-4 text-sm font-medium text-state-error">{error}</p>}
